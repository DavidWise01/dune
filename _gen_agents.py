#!/usr/bin/env python3
"""Materialize D1 (Dune) persona ACI badges from the .agent files on disk.
Each → full ACI complement + agents/_personas.json with emergence-nature."""
import os, sys, json, re
sys.stdout.reconfigure(encoding="utf-8")
HERE = os.path.dirname(os.path.abspath(__file__))
sys.path.insert(0, HERE)
import build  # dune/build.py — write_aci, NATURES

AGENTS = os.path.join(HERE, "agents")
ORDER = ["paul-atreides","lady-jessica","duke-leto-atreides","gurney-halleck","duncan-idaho","thufir-hawat",
         "chani","stilgar",
         "baron-harkonnen","feyd-rautha","emperor-shaddam-iv","princess-irulan","reverend-mother-mohiam",
         "alia-atreides","leto-ii","ghanima","scytale",
         "miles-teg","darwi-odrade","sheeana","murbella",
         "the-kwisatz-haderach","the-golden-path","the-bene-gesserit","the-fremen","the-honored-matres",
         "the-spice-melange","shai-hulud","the-spacing-guild","the-mentats"]
...
for fn in os.listdir(AGENTS):
    ...
    rec = {
        "name": fr.get("aci", slug), "axiom": "D1", "emergence": em,
        "seal": fr.get("seal", ""), "origin": "D1 · Dune",
        ...
        "conductor": "ROOT0 (catalogued into UD0)", "inputs": fr.get("domain", "Dune"),
        "source": "Dune emergent, catalogued by ROOT0",
    }
...
print(f"wrote {len(ordered)} D1 persona ACI badges + _personas.json")
...
for r in ordered:
    print(f"  {r['slug']:26} {r['emergence']:10} {r['moniker']}")
